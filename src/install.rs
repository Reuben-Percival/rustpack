...
pub fn install_packages(packages: &[String], global: &GlobalFlags) -> Result<()> {
    ...
    if global.verbose {
        println!(
            ":: verbose: operation=install targets={}",
            packages.join(" ")
        );
    }
...
pub fn install_local(global: &GlobalFlags, pkg_files: &[String]) -> Result<()> {
    ...
    if global.verbose {
        println!(
            ":: verbose: operation=install-local files={}",
            pkg_files.join(" ")
        );
    }
...
pub fn remove_packages(
    ...
    if global.verbose {
        println!(
            ":: verbose: operation=remove targets={}",
            packages.join(" ")
        );
    }
...
pub fn sync_install(
    ...
    if global.verbose {
        println!(
            ":: verbose: operation=sync refresh={} upgrade={} targets={}",
            refresh,
            upgrade,
            targets.join(" ")
        );
...
pub fn clean_cache(global: &GlobalFlags, level: u8) -> Result<()> {
    ...
    for entry in fs::read_dir(cache_path)? {
        ...
        let remove = if level >= 2 {
            ...
            match parse_pkg_filename(file_name) {
                Some((name, version)) => match localdb.pkg(name.as_str()) {
    ...
    if removed > 0 {
        println!(
            ":: {} {}",
            "Cache cleaned:".green().bold(),
            format!("{} files removed", removed)
        );
    } else {
...
pub fn set_install_reason(packages: &[String], global: &GlobalFlags) -> Result<()> {
    let handle = alpm_ops::init_handle(global)?;
    ...
    let reason_label = if global.asdeps {
        "dependency"
    } else {
        "explicit"
    };
    if global.verbose {
        println!(
            ":: verbose: operation=set-reason reason={} targets={}",
            reason_label,
            packages.join(" ")
        );
    }
    if global.test {
        if !global.compact {
            println!(":: {}", "--test: skipping reason changes".yellow());
        }
        let _ = history::record(
            global,
            "database",
            "dry-run",
            packages,
            "reason changes skipped by --test",
        );
        return Ok(());
    }
    let localdb = handle.localdb();
    for name in packages {
        let pkg = localdb
            .pkg(name.as_str())
            .map_err(|_| anyhow::anyhow!("error: package '{}' was not found", name))?;
        pkg.set_reason(reason)?;
        if !global.compact {
            println!(
                "{} {} {}",
                "Set install reason:".cyan().bold(),
                pkg.name().green().bold(),
                reason_label
            );
        }
    ...
    let _ = history::record(
        ...
        "success",
        packages,
        "updated package install reasons",
    );
    Ok(())
}
